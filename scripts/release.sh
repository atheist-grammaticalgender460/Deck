#!/bin/bash
# Build Deck into a distributable, AD-HOC-signed .app + .dmg for GitHub Releases.
#
# NOT notarized, tied to no Apple Developer account — like most open-source Mac
# apps. On first launch users approve it once via System Settings → Privacy &
# Security → "Open Anyway". In-app updates still work: Sparkle verifies the
# EdDSA-signed appcast, independent of Apple notarization.
#
...
DEVELOPER_DIR="$DEV" xcodebuild -project Deck.xcodeproj -scheme Deck \
  -configuration Release -derivedDataPath build \
  ARCHS="arm64 x86_64" ONLY_ACTIVE_ARCH=NO CODE_SIGNING_ALLOWED=NO build >/dev/null

APP_SRC="build/Build/Products/Release/Deck.app"
[ -d "$APP_SRC" ] || { echo "error: build product missing" >&2; exit 1; }
rm -rf "$DIST"; mkdir -p "$DIST"
APP="$DIST/Deck.app"
cp -R "$APP_SRC" "$APP"

echo "→ Ad-hoc signing (inside-out, incl. Sparkle)…"
...
cp -R "$APP" "$STAGING/Deck.app"
ln -s /Applications "$STAGING/Applications"
hdiutil create -volname "Deck" -srcfolder "$STAGING" -ov -format UDZO "$DMG" >/dev/null
rm -rf "$STAGING"
...
if [ -x "$GENAPPCAST" ] && [ -f "$KEYFILE" ]; then
  ...
  "$GENAPPCAST" --ed-key-file "$KEYFILE" \
    --download-url-prefix "https://github.com/hatimhtm/Deck/releases/download/v$VERSION/" \
    "$APPCAST_SRC"
  ...
  if [ -f "$CHANGELOG" ]; then
    ...
    if [ -s "$NOTES_MD" ]; then
      ...
      {
        echo "<h3 style=\"margin:0 0 8px;font:600 14px -apple-system\">What&rsquo;s new in Deck $VERSION</h3>"
...
if [ -n "$APPCAST" ]; then
  echo "  gh release create v$VERSION \"$DMG\" \"$ZIP\" \"$APPCAST\" --title \"Deck v$VERSION\" $NOTES_ARG"
else
  echo "  gh release create v$VERSION \"$DMG\" \"$ZIP\" --title \"Deck v$VERSION\" $NOTES_ARG"
fi
